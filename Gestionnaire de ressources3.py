...
class DefVal(Root):
        ...
        for i in range(4):
            ...
            if self.modifie:
                if i == 0:
                    ...
                    self._creation_label(
                        frame=self.frame, name=self.liste_nom[i])
                    self._creation_entry(
                        frame=self.frame, name=self._valeurs[i])
            else:
                self._creation_label(frame=self.frame, name=self.liste_nom[i])
    ...
    def _test_encadrement(self, liste) -> bool:
        ...
        # If test is verified
        if test:
            # We loop on ressource values
            for index in range(len(self.valeurs)):
                # When it is not the index of name value
                if index != 0:
                    self.valeurs[index] = int(self.valeurs[index])
            # If the name is empty we stop the loop
            if liste[0] == "":
                return False
            if liste[1] > liste[2]:
